            ...
            <section class="card">
              ...
              <div class="card-body">
                ...
                <form id="builder-form">
                  ...
                  <div class="tracking-loading-shell" id="tracking-loading-shell" aria-busy="false">
                    <div class="tracking-loading-overlay" id="tracking-loading-overlay" hidden role="status" aria-live="polite">
                      <span class="loading-spinner" aria-hidden="true"></span>
                      <strong>Loading recommended values…</strong>
                    </div>
...
function renderStyles() {
  return `${renderJustFlowShellStyles()}
    .tracking-loading-shell{position:relative}.tracking-loading-overlay{position:absolute;inset:-6px;z-index:60;display:flex;align-items:flex-start;justify-content:center;gap:9px;padding-top:34px;border-radius:var(--radius);background:rgba(255,255,255,.82);color:var(--text);backdrop-filter:blur(2px)}[data-theme=dark] .tracking-loading-overlay{background:rgba(17,17,20,.84)}.tracking-loading-overlay[hidden]{display:none}.loading-spinner{width:17px;height:17px;flex:0 0 auto;border:2px solid var(--border-strong);border-top-color:var(--accent);border-radius:50%;animation:loading-spin .7s linear infinite}.suggestion-loading{display:flex;align-items:center;gap:8px;padding:9px 10px;color:var(--text-2);font-size:12px}.suggestion-loading .loading-spinner{width:14px;height:14px}@keyframes loading-spin{to{transform:rotate(360deg)}}@media (prefers-reduced-motion:reduce){.loading-spinner{animation-duration:1.5s}}
    .builder-layout>.card,.builder-layout>.card>.card-body{overflow:visible}.combo-card:focus-within{z-index:80}
...
function renderClientScript(clients = []) {
  return `(function(){
    ...
    const trackingLoadingShell=document.getElementById("tracking-loading-shell");
    const trackingLoadingOverlay=document.getElementById("tracking-loading-overlay");
    ...
    const state={requestNonce:0,recommendationsLoading:false,pendingConsistencyFingerprint:null,confirmedConsistencyFingerprint:null,pendingConsistencyWarnings:[],currentConsistencyWarnings:[]};
    const disabledBeforeLoading=new Map();
    ...
    function setRecommendationLoading(loading){state.recommendationsLoading=loading;trackingLoadingShell.setAttribute("aria-busy",loading?"true":"false");trackingLoadingOverlay.hidden=!loading;const controls=[clientSelect].concat(UTM_FIELDS.map((field)=>inputNodes[field]),submitButton);if(loading){controls.forEach((control)=>{disabledBeforeLoading.set(control,control.disabled);control.disabled=true})}else{controls.forEach((control)=>{control.disabled=disabledBeforeLoading.get(control)===true});disabledBeforeLoading.clear()}}
    async function withRecommendationLoading(work,nextField){if(state.recommendationsLoading){return}let completed=false;setRecommendationLoading(true);try{await work();completed=true}catch(error){showStatus(error&&error.message?error.message:"Unable to load recommended values.","error")}finally{setRecommendationLoading(false)}if(completed&&nextField&&inputNodes[nextField]){inputNodes[nextField].focus()}}
    ...
    function renderSuggestions(field,items,typedValue){suggestionState[field]=items||[];renderKnownState(field);const list=listNodes[field];const typed=String(typedValue||"").trim();const typedComparable=comparableUtmValue(typed);const exactKnown=(items||[]).some((item)=>comparableUtmValue(item.normalized_value||item.value)===typedComparable);const rows=[];if(typed&&!exactKnown){rows.push('<button type="button" class="suggestion-button" data-field="'+escapeAttribute(field)+'" data-value="'+escapeAttribute(typed)+'"><span class="suggestion-main"><strong>Use new value</strong><span class="suggestion-help">'+escapeHtml(typed)+'</span></span><span class="pill warning">New</span></button>')}(items||[]).forEach((item)=>{const badge=item.recommended?'Recommended':(item.known?'Known':'New');const badgeClass=item.recommended?'success':(item.known?'success':'warning');rows.push('<button type="button" class="suggestion-button" data-field="'+escapeAttribute(field)+'" data-value="'+escapeAttribute(item.value)+'"><span class="suggestion-main"><strong>'+escapeHtml(item.value+" ("+String(item.count||0)+")")+'</strong>'+(item.relation?'<span class="suggestion-help">'+escapeHtml(item.relation)+'</span>':"")+'</span><span class="pill '+badgeClass+'">'+badge+'</span></button>')});if(!rows.length){list.innerHTML="";list.classList.remove("visible");return}list.innerHTML=rows.join("");list.classList.add("visible")}
    async function loadSuggestions(field,query,showList){const requestId=++suggestionRequestIds[field];const previousItems=suggestionState[field]||[];if(!clientSelect.value){if(showList){renderSuggestions(field,[],query)}return}if(showList){renderSuggestionLoading(field)}const selection=currentSelection();try{const body=await fetchJson("/new/utm-intelligence/suggestions.json?"+queryString({field,client:selection.client,campaign:selection.campaign,source:selection.source,medium:selection.medium,term:selection.term,content:selection.content,query}));if(requestId!==suggestionRequestIds[field]){return}if(showList){renderSuggestions(field,body.items||[],query)}else{suggestionState[field]=body.items||[];renderKnownState(field)}}catch(error){if(requestId!==suggestionRequestIds[field]){return}if(showList){renderSuggestions(field,previousItems,query)}throw error}}
    ...
    document.addEventListener("click",async(event)=>{const confirmButton=event.target.closest("[data-confirm-consistency]");if(confirmButton){event.preventDefault();if(state.recommendationsLoading){return}state.confirmedConsistencyFingerprint=state.pendingConsistencyFingerprint;form.requestSubmit();return}const reviewButton=event.target.closest("[data-review-consistency]");if(reviewButton){event.preventDefault();const field=state.pendingConsistencyWarnings.flatMap((item)=>item.fields||[])[0];if(field&&inputNodes[field]){inputNodes[field].focus()}return}const suggestion=event.target.closest("[data-field][data-value]");if(suggestion){event.preventDefault();if(state.recommendationsLoading){return}const field=suggestion.getAttribute("data-field");inputNodes[field].value=suggestion.getAttribute("data-value")||"";state.confirmedConsistencyFingerprint=null;state.pendingConsistencyFingerprint=null;closeSuggestions(field);updateAdvancedSummary();updateChannelSummary();const fieldIndex=UTM_FIELDS.indexOf(field);const nextField=fieldIndex>=0&&fieldIndex<UTM_FIELDS.length-1?UTM_FIELDS[fieldIndex+1]:null;await withRecommendationLoading(async()=>{await loadSuggestions(field,inputNodes[field].value.trim(),false);if(field==="campaign"){updateClientGuidance();await Promise.all([loadSuggestions("source",inputNodes.source.value.trim(),false),loadSuggestions("medium",inputNodes.medium.value.trim(),false),loadSuggestions("term",inputNodes.term.value.trim(),false),loadSuggestions("content",inputNodes.content.value.trim(),false)])}if(field==="source"){await loadSuggestions("medium",inputNodes.medium.value.trim(),false)}renderLoadedSuggestions();await refreshContextAndPreview()},nextField);return}const copyButton=event.target.closest("[data-copy]");if(copyButton){event.preventDefault();try{await navigator.clipboard.writeText(copyButton.getAttribute("data-copy")||"");showStatus("Copied to clipboard.","success")}catch{showStatus("Copy failed.","error")}return}if(!event.target.closest(".combo-card")){closeAllSuggestions()}});
    clientSelect.addEventListener("change",()=>{state.confirmedConsistencyFingerprint=null;state.pendingConsistencyFingerprint=null;updateChannelSummary();updateClientGuidance();withRecommendationLoading(async()=>{await Promise.all(UTM_FIELDS.map((field)=>loadSuggestions(field,inputNodes[field].value.trim(),false)));renderLoadedSuggestions();await loadClientHistory();await refreshContextAndPreview()},clientSelect.value?"campaign":null)});
    destinationInput.addEventListener("input",()=>{updateDestinationQueryNotice();debouncedRefresh()});
    ...
    updateQrProjectPicker();updateDestinationQueryNotice();updateChannelSummary();updateAdvancedSummary();updateClientGuidance();UTM_FIELDS.forEach((field)=>renderSuggestions(field,[],""));refreshContextAndPreview().catch(()=>{})
  })();`;
}
