...
function renderClientScript(clients = []) {
  return `(function(){
    ...
    const state={requestNonce:0,recommendationLoadToken:0,activeRecommendationField:null,queuedSubmission:false,submitting:false,pendingConsistencyFingerprint:null,confirmedConsistencyFingerprint:null,pendingConsistencyWarnings:[],currentConsistencyWarnings:[]};
    ...
    function beginRecommendationLoading(field){if(!field||state.activeRecommendationField){return null}const token=++state.recommendationLoadToken;state.activeRecommendationField=field;comboCardNodes[field].setAttribute("aria-busy","true");fieldLoadingNodes[field].hidden=false;inputNodes[field].disabled=true;submitButton.disabled=true;return token}
    function finishRecommendationLoading(field,token){if(!field||state.activeRecommendationField!==field||state.recommendationLoadToken!==token){return false}comboCardNodes[field].setAttribute("aria-busy","false");fieldLoadingNodes[field].hidden=true;inputNodes[field].disabled=false;state.activeRecommendationField=null;submitButton.disabled=state.submitting;return true}
    async function withRecommendationLoading(work,nextField){if(!nextField){try{await work()}catch(error){showStatus(error&&error.message?error.message:"Unable to load recommended values.","error")}return}const token=beginRecommendationLoading(nextField);if(token===null){return}let completed=false;try{await work();completed=true}catch(error){showStatus(error&&error.message?error.message:"Unable to load recommended values.","error")}finally{const cleared=finishRecommendationLoading(nextField,token);if(cleared&&state.queuedSubmission){state.queuedSubmission=false;form.requestSubmit();return}}if(completed&&inputNodes[nextField]){inputNodes[nextField].focus()}}
