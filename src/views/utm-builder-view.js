...
function renderClientScript(clients = []) {
  return `(function(){
    ...
    function renderWarnings(preview,context){const consistency=context&&context.consistency?context.consistency:(preview&&preview.context?preview.context.consistency:null);const structured=consistency&&consistency.warnings?consistency.warnings:[];const legacy=[].concat((preview&&preview.resolved&&preview.resolved.warnings)||[]).map((warning)=>typeof warning==="string"?{type:"guidance",severity:"warning",message:warning,recommendations:[]}:{type:warning&&warning.type?warning.type:"guidance",severity:warning&&warning.severity?warning.severity:"warning",message:warning&&warning.message?warning.message:"",recommendations:warning&&warning.recommendations?warning.recommendations:[]}).filter((warning)=>warning.message);const messages=new Set(structured.map((item)=>item.message));const combined=structured.concat(legacy.filter((warning)=>!messages.has(warning.message)));renderConsistencyWarnings(combined,false)}
    ...
    UTM_FIELDS.forEach((field)=>{inputNodes[field].addEventListener("focus",()=>{loadSuggestions(field,inputNodes[field].value.trim(),true).catch(()=>{})});inputNodes[field].addEventListener("blur",()=>{const formatted=formatUtmInput(inputNodes[field].value);if(formatted&&formatted!==inputNodes[field].value){inputNodes[field].value=formatted;inputNodes[field].dispatchEvent(new Event("input",{bubbles:true}))}});inputNodes[field].addEventListener("input",async()=>{state.confirmedConsistencyFingerprint=null;state.pendingConsistencyFingerprint=null;updateAdvancedSummary();updateChannelSummary();if(field==="campaign"){updateClientGuidance()}await loadSuggestions(field,inputNodes[field].value.trim(),true);if(field==="campaign"){await Promise.all([loadSuggestions("source",inputNodes.source.value.trim(),false),loadSuggestions("medium",inputNodes.medium.value.trim(),false),loadSuggestions("term",inputNodes.term.value.trim(),false),loadSuggestions("content",inputNodes.content.value.trim(),false)])}if(field==="source"){await loadSuggestions("medium",inputNodes.medium.value.trim(),listNodes.medium.classList.contains("visible"))}debouncedRefresh()})});
    form.addEventListener("reset",()=>{window.setTimeout(()=>{closeAllSuggestions();showStatus("","");resultShell.classList.remove("visible");updateDestinationQueryNotice();updateChannelSummary();updateAdvancedSummary();updateClientGuidance();UTM_FIELDS.forEach((field)=>renderSuggestions(field,[],""));refreshContextAndPreview().catch(()=>{})},0)});
    ...
    updateDestinationQueryNotice();updateChannelSummary();updateAdvancedSummary();updateClientGuidance();UTM_FIELDS.forEach((field)=>renderSuggestions(field,[],""));refreshContextAndPreview().catch(()=>{})
  })();`;
}
