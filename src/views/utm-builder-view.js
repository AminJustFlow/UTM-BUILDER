...
function renderClientScript(clients = []) {
  return `(function(){
    ...
    function setRecommendationLoading(field,loading){state.recommendationsLoading=loading;if(!field){return}comboCardNodes[field].setAttribute("aria-busy",loading?"true":"false");fieldLoadingNodes[field].hidden=!loading;inputNodes[field].disabled=loading;submitButton.disabled=loading}
    async function withRecommendationLoading(work,nextField){if(state.recommendationsLoading){return}let completed=false;setRecommendationLoading(nextField,true);try{await work();completed=true}catch(error){showStatus(error&&error.message?error.message:"Unable to load recommended values.","error")}finally{setRecommendationLoading(nextField,false)}if(completed&&nextField&&inputNodes[nextField]){inputNodes[nextField].focus()}}
    ...
    function formatUtmInput(value){const text=String(value||"").trim();if(!text){return ""}const preservedWords={cpc:"CPC",gmb:"GMB",jf:"JF",ma:"MA",nh:"NH",pr:"PR",qr:"QR",seo:"SEO",utm:"UTM"};const preservedPhrases={linkedin:"LinkedIn"};const phrase=preservedPhrases[text.toLowerCase()];if(phrase){return phrase}return text.replace(/([a-z0-9])([A-Z])/g,"$1 $2").split(/[^a-zA-Z0-9]+/g).filter(Boolean).map((token)=>{const normalized=token.toLowerCase();return preservedWords[normalized]||preservedPhrases[normalized]||normalized.charAt(0).toUpperCase()+normalized.slice(1)}).join("")}
    ...
    UTM_FIELDS.forEach((field)=>{inputNodes[field].addEventListener("blur",async()=>{const rawValue=inputNodes[field].value.trim();const applyFormatted=(value)=>{if(value===inputNodes[field].value){return}inputNodes[field].value=value;state.confirmedConsistencyFingerprint=null;state.pendingConsistencyFingerprint=null;updateAdvancedSummary();updateChannelSummary();if(field==="campaign"){updateClientGuidance()}};const profile=field==="campaign"?activeCampaignProfile(CLIENT_GUIDANCE[clientSelect.value]||{}):null;const cachedKnown=(suggestionState[field]||[]).find((item)=>item.known&&comparableUtmValue(item.normalized_value||item.value)===comparableUtmValue(rawValue));applyFormatted(profile&&profile.campaign?profile.campaign:(cachedKnown&&cachedKnown.value?cachedKnown.value:formatUtmInput(rawValue)));try{await loadSuggestions(field,rawValue,false);const known=(suggestionState[field]||[]).find((item)=>item.known&&comparableUtmValue(item.normalized_value||item.value)===comparableUtmValue(rawValue));if(known&&known.value){applyFormatted(known.value)}renderLoadedSuggestions();await refreshContextAndPreview()}catch(error){debouncedRefresh();showStatus(error.message,"error")}})});
