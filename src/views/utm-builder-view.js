...
function renderClientScript(clients = []) {
  return `(function(){
    ...
    const state={requestNonce:0,pendingConsistencyFingerprint:null,confirmedConsistencyFingerprint:null,pendingConsistencyWarnings:[],currentConsistencyWarnings:[]};
    ...
    renderResult=function(payload){const cleanUtms=state.currentConsistencyWarnings.length===0&&!(payload.warnings||[]).length;document.getElementById("result-celebration").hidden=!cleanUtms;renderResultBase(payload);if(cleanUtms){document.getElementById("result-title").textContent="UTM success!"}};
